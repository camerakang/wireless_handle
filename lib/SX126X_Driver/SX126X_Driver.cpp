...
void SX126xCheckDeviceReady(void)
{
  ...
  {
    Serial.println("MODE_SLEEP  or  MODE_RX_DC");
    SX126xWakeup();
  }
  SX126xWaitOnBusy();
}
...
void SX126X_Reset(void)
{
  ...
  delay(10);
  Serial.println("SX126X_Reset end");
}
/**
 * @简介：SX126X  IO初始化及配置
 * @参数：无
...
void SX126xSetStandby(RadioStandbyModes_t standbyConfig)
{
  ...
  Serial.printf("standbyConfig = 0x%x\n",standbyConfig);
  ...
  Serial.println("SX126xSetStandby end");
}
...
tSX126xError SX126x_Lora_init(void)
{
  ...
  {
    Serial.println(" tSX126xError:::PARAMETER_INVALID!!!!");
  ...
  SPI 验证                   */
  uint8_t test = 0;
  SX126xWriteRegister(REG_LR_PAYLOADLENGTH, 0xaa); // 选一个用不到的寄存器来做验证
  test = SX126xReadRegister(REG_LR_PAYLOADLENGTH);
  Serial.printf("test read date = 0x%x\n", test);
  if (test != 0xaa)
  {
    // Serial.printf("tSX126xError:::SPI_READCHECK_WRONG!!!! \n");
    return SPI_READCHECK_WRONG;
  }
  // Serial.printf("test finished \n");
...
void LSD_RF_SendPacket(uint8_t *cbuf)
{
  ...
  while ((DIO1_GetState() == GPIO_PIN_RESET) && j)
    j--; // 等待DIO1电平为高
  //  __HAL_GPIO_EXTI_CLEAR_IT(GPIO_NUM_1);
}
